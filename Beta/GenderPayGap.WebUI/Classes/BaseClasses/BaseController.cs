...
namespace GenderPayGap
{
    public class BaseController : Controller
    {
        ...
        protected ActionResult CheckUserRegisteredOk(out User currentUser)
        {
            ...
            else if (IsAnyAction("Register/ReviewRequest", "Register/ConfirmCancellation", "Register/RequestAccepted", "Register/RequestCancelled"))
        ...
        public ActionResult ImpersonateUser(string emailAddress)
        {
            var user = DataRepository.FindUserByEmail(emailAddress);
            if (user == null) return null;

            var claims = new List<Claim>();
            claims.Add(new Claim(Constants.ClaimTypes.Subject, user.UserId.ToString()));
            var impersonatedIdentity = new ClaimsIdentity(claims, CookieAuthenticationDefaults.AuthenticationType);

            HttpContext.GetOwinContext().Authentication.SignIn(new AuthenticationProperties{IsPersistent = false}, impersonatedIdentity);

            //Remove the email address from the querystring
            var newRoute = new RouteValueDictionary();

            foreach (var key in Request.QueryString.AllKeys)
            {
                if (!Request.QueryString[key].EqualsI(emailAddress))
                    newRoute.Add(key, Request.QueryString[key]);
            }

            //Refresh page to ensure identity is passed in cooke
            return RedirectToAction(ActionName,ControllerName, newRoute);
        }
    }
}